...
> > * [重新命名](#重新命名)
> 
> 点击配置文件的 `ⓘ` 图标或者点击 “编辑配置” 进入配置文件编辑菜单：
> > * [通用](#通用参数)
> > * [规则](#添加规则)
> > * [Hosts](#hosts)
> > * [URL重写](#url重写)
...
> 点击 **使用配置** 可以 [编译](#编译配置) 并启用该配置文件。该功能会对该配置使用的远程 [规则集](#规则集url)、[脚本](#脚本url) 等远程资源进行更新
...
### [编译配置](#使用目录)
 ...
 > 点击 **编译配置** 可以对该配置文件进行重新编译。可以理解为软件将该配置文件编辑并转译为符合当前版本规范和逻辑的底层使用格式。该功能会更新该配置文件所用到的 [规则集](#规则集url)、[脚本](#脚本url) 等远程资源
